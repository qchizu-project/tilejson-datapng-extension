...
`encoding` は、ピクセルの RGB 値からデータ値を復号する方式を指定する。**本拡張における数値PNGの正式かつ推奨されるエンコーディングは `"datapng"`（符号付き24ビット整数）である。** データPNGは数値タイルPNGの元祖となる仕組みであり、標高に限らず任意の数値を符号付きで表現でき、`factor`・`offset`・`unit` による柔軟な単位変換に対応するなど、標高専用の後発エンコード方式に対して優位性を持つ。本拡張の目的は、このデータPNGの普及にある。`"mapbox"`・`"terrarium"` は後発の標高RGBエンコード方式であり、それらで配信された既存タイルとの相互運用・移行を可能にするため、**互換エンコーディング**として読み取りに対応する。
...
後発の標高RGBタイル形式（Mapzen/Terrarium）を互換エンコーディングで取り込む例。復号式は固定のため `factor`・`offset` は指定しない。

```json
{
  "tilejson": "3.0.0",
  "name": "AWS Terrain Tiles (Terrarium)",
  "description": "AWS Open Data の全球標高タイル（Mapzen/Terrarium 形式）。Vertical datum: EGM2008 geoid height.",
  "attribution": "Mapzen, AWS, and data providers",
  "tiles": [
