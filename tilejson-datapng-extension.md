...
`encoding` は、ピクセルの RGB 値からデータ値を復号する方式を指定する。**本拡張における数値PNGの正式なエンコーディングは `"datapng"`（符号付き24ビット整数）である。** `"mapbox"`・`"terrarium"` は、これらの方式で配信された既存タイルとの相互運用のために対応する**互換エンコーディング**である。
