...
- タイル画像の**標準形式は WebP（可逆圧縮）**とし、PNG も許容する（§2.2）。「データPNG」という名称と `datapng` キーは先行するデータPNG仕様との連続性のために維持するが、**格納形式は PNG に限らない**。形式はタイル URL の拡張子が示すため、専用のフィールドは設けない。
...
グリッドデータタイルの**標準形式は WebP（可逆圧縮）**とする。可逆圧縮のまま PNG より配信容量を削減でき、主要なブラウザがすべて対応しているためである。PNG での配信も許容する。

いずれも可逆圧縮であり、格納された値は劣化しない。**非可逆な形式（JPEG 等）で配信してはならない**（MUST NOT）——画素値が変わり、復号した値が別の値になる。

**形式を示す専用のフィールドは設けない。** TileJSON 3.0.0 は `tiles` の項で「タイルの拡張子は特定の形式に限定されない（png, webp 等）」と述べており、形式は**タイル URL の拡張子**が担っている。専用フィールドを足しても、実装は結局取得したバイト列で復号することになる（宣言と実体が食い違いうるため）。

クライアントは、宣言や拡張子ではなく**実際に取得したバイト列の形式で復号しなければならない**（MUST）。
...
{
  "tilejson": "3.0.0",
  "tiles": [ "https://example.org/tiles/elev/{z}/{x}/{y}.webp" ],
  "datapng": {
    "type": "numerical",
    "factor": 0.01,
    "unit": "m",
...
3. `datapng.type` に応じたデコーダを選択する。タイル画像の形式はタイル URL の拡張子が示すが、復号は実際に取得したバイト列の形式に従う（§2.2）。
