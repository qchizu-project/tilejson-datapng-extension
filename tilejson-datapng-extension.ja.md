...
- 「データPNG」という名称であるが、格納形式は PNG のほか **WebP（可逆圧縮）**も使用できる。WebP を標準とする。
...
2. **invalidColor チェック**: `invalidColor` が指定されている場合、**完全に透明でないピクセル**について、RGB 値が `invalidColor` と完全一致するかを判定する。一致した場合は無効値とする。
